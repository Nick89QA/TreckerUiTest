...
4. Запустить терминал из корневой папки проекта и выполнить команду: 
    ```
    ./start_docker.sh
    ```
...
необходимо выполнить команду в Git bash:
    ```
    dos2unix.exe start_local.sh
    ```
либо переключить формат для всех sh файлов с CRLF на LF и сохранить их.
